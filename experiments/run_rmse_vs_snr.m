...
rmse_r = nan(size(SNR_dB_Range)); rmse_v = nan(size(SNR_dB_Range));
...
results = struct('cfg', cfg, 'seed', cfg.seed, 'snr_db', SNR_dB_Range, ...
    'rmse_range_idx', rmse_r, 'rmse_doppler_idx', rmse_v, ...
    'timestamp', manifest.timestamp, 'git_commit_hash', manifest.git_commit_hash, 'profile', profile, 'invariance_report', invariance_report);
save(fullfile(cfg.experiments.output_dir, 'rmse_vs_snr.mat'), 'results', 'manifest');
end
